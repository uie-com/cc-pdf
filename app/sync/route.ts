...
export const CALENDAR_CREATE_TIMEOUT = 1000 * 60 * 5;
...
export const EVENT_FIND_TIMEOUT = 1000 * 1 / 2;
export const EVENT_CREATE_TIMEOUT = 1000 * 1;
export const EVENT_UPDATE_TIMEOUT = 1000 * 1 / 2;
...
export async function GET(request: NextRequest) {
    ...
    // sessions = sessions.filter(session => session.fields && session.fields['Name (from Cohort Identifier)'].join('').includes('Stand Out'));
